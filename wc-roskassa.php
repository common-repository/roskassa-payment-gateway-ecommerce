    ...
    function woocommerce_roskassa()
    {
        ...
        class WC_ROSKASSA extends WC_Payment_Gateway
        {
            public function __construct()
            {
                ...
                $this->icon = apply_filters('woocommerce_roskassa_icon', $plugin_dir . 'assets/roskassa_logo_blue.png');
        ...
        function check_ipn_response()
        {
            ...
            else if (isset($_GET['roskassa']))
            {
                ...
                if ($_GET['roskassa'] == 'success') {

                    $order->payment_complete();
                    $order->add_order_note(__('Платеж успешно оплачен через Роскассу', 'roskassa'));

                } elseif ($_GET['roskassa'] == 'fail') {

                    $order->update_status('failed', __('Платеж не оплачен', 'roskassa'));
                    $order->add_order_note(__('Платеж не оплачен', 'roskassa'));
